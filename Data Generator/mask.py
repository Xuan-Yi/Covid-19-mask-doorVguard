...
class FaceMasker:
    ...
    def _mask_face(self, face_landmark: dict):
        ...
        rotated_mask_img = mask_img.rotate(90-angle*180.0/np.pi, expand=True)
        ...
        # add mask
        self._face_img.paste(
            rotated_mask_img, (box_x, box_y), rotated_mask_img)

    def _save(self):
        path_splits = os.path.split(self.face_path)
        # folder path to save new face images
        new_folder_path = os.path.join(
            os.getcwd(), "Data Generator\data", "with_"+os.path.splitext(os.path.basename(self.mask_path))[0])
        if not os.path.isdir(new_folder_path):
            os.mkdir(new_folder_path)
            
        new_face_path = os.path.join(new_folder_path, path_splits[1])
